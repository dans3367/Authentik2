    ...
    <div className="flex items-center gap-2 min-w-[120px]">
      <div className="flex-1 h-1.5 rounded-full bg-muted/70 overflow-hidden">
        <div
          className={cn(
            "h-full rounded-full",
            tone === "accent" ? "bg-rose-500" : "bg-foreground/80",
          )}
          style={{ width: `${pct}%` }}
        />
      </div>
...
export function NewsletterStatsCard() {
  ...
  return (
    <Card className="h-full rounded-2xl border-border/50 flex flex-col">
      ...
      <CardContent className="px-5 pb-5 pt-0 flex-1 flex flex-col">
        <div className="flex items-center justify-between gap-2 mb-3">
          ...
          <span className="text-[10px] font-semibold text-muted-foreground/60 bg-muted/60 px-2 py-1 rounded-md whitespace-nowrap">
            {total} newsletters
          </span>
        </div>
              ...
              <tbody>
                {rows.map((item) => {
                  ...
                  return (
                    ...
                    >
                      <td className="py-3 pr-4 border-t border-border/40">
                      ...
                      <td className="py-3 pr-4 border-t border-border/40 text-right font-mono text-xs tabular-nums text-foreground/80">
                        {item.recipientCount
                          ? formatLongNumber(item.recipientCount)
                          : <span className="text-muted-foreground/50">—</span>}
                      </td>
                      <td className={cn("py-3 pr-4 border-t border-border/40", dim && "opacity-60")}>
                        <RateBar value={openRate} tone="neutral" />
                      </td>
                      <td className={cn("py-3 pr-4 border-t border-border/40", dim && "opacity-60")}>
                        <RateBar value={clickRate} tone="accent" />
                      </td>
                      <td className="py-3 border-t border-border/40 text-right font-mono text-[11px] text-muted-foreground/70 whitespace-nowrap">
                        {formatDate(item)}
                      </td>
                    </tr>
                  );
