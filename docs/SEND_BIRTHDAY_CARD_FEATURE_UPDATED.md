...
## Overview
This feature allows users to manually send birthday cards to selected customers through the **Birthdays page** (`/birthdays?tab=customers`). The feature is accessible via a "Send Birthday Card" button in the ellipse menu dropdown when one or more customers are selected.

## ✅ Correct Location
**Page:** `/birthdays?tab=customers` (Birthdays page - Customers tab)
**NOT** the Email Contacts page
...
**File:** `client/src/pages/birthdays.tsx`

- **Location:** Birthdays page (`/birthdays?tab=customers`)
- **Tab:** Customers tab
- **Menu:** Ellipse (three dots) dropdown menu
- **Position:** FIRST item in dropdown (above "Export Selected")
- **Icon:** Cake icon (CakeIcon from lucide-react)
- **States:**
  - Default: "Send Birthday Card"
  - Loading: "Sending..." (menu item disabled during operation)
...
```
┌─────────────────────────────┐
│ 🎂 Send Birthday Card       │ ← NEW (First item)
├─────────────────────────────┤
│ 📥 Export Selected          │
├─────────────────────────────┤
│ 🗑️ Remove Selected          │
...
- Shows count of selected customers in confirmation
- Displays success/failure counts in toast notification
- Clears selection after successful send
- Prevents interaction during send with disabled state
- Uses i18n for localization support
...
1. User navigates to Birthdays page (`/birthdays?tab=customers`)
2. User selects one or more customers via checkboxes
3. User clicks the ellipse menu (three dots icon)
4. User clicks "Send Birthday Card" (first option in dropdown)
...
4. **UI/UX:**
   - Button appears in ellipse menu only when customers selected
   - Confirmation dialog works correctly
   - Loading state displays properly (menu item disabled + "Sending..." text)
...
| Access Point | N/A (automated) | Birthdays page → Customers tab |
...
2. `client/src/pages/birthdays.tsx` - Added button, mutation, and handler
...
- Button is in ellipse menu dropdown for space efficiency
- Position: First item (above Export Selected) for easy access
- Uses existing "selectedContacts" state from the customers tab
