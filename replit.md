...
## Recent Changes
- **2026-03-08**: Security fixes across two commits (80452c8 and 7da867b): (1) Preview endpoint now requires authentication + tenant membership check to prevent IDOR access to drafts. (2) Newsletter HTML content sanitized via sanitizeEmailHtml() on all public endpoints. (3) Rate limiting (60 req/min) added to public newsletter routes. (4) Unique partial index on (tenant_id, web_slug) added to prevent duplicate slugs. (5) BirthdayWorker email logging now uses maskEmail() to prevent PII leaks. (6) Bounce check endpoint uses consistent LOWER() case handling. (7) publishedAt/webSlug removed from updateNewsletterSchema to prevent manual publish bypass. (8) Full bouncedEmail record removed from bounce check API response.
