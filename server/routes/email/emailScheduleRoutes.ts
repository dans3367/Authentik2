...
import { sanitizeEmailHtml, sanitizeFontFamily, isValidHttpUrl, escapeHtml, maskEmail } from './emailUtils';

export const emailScheduleRoutes = Router();

// List all upcoming scheduled emails for the tenant (dashboard widget)
emailScheduleRoutes.get("/scheduled-emails/upcoming", authenticateToken, requireTenant, requirePermission('contacts.view'), asyncHandler(async (req: any, res) => {
...
emailScheduleRoutes.get("/email-contacts/:id/scheduled", authenticateToken, requireTenant, requirePermission('contacts.view'), asyncHandler(async (req: any, res) => {
  const { id } = req.params;
  const tenantId = req.user.tenantId;
...
emailScheduleRoutes.post("/email-contacts/:id/send-email", authenticateToken, requireTenant, requirePermission('contacts.edit'), (req: any, res: any, next: any) => {
  // Only run multer for multipart/form-data requests (when attachments are present)
  const contentType = req.headers['content-type'] || '';
  if (contentType.includes('multipart/form-data')) {
    emailAttachmentUpload(req, res, (err: any) => {
