...
func main() {
	...
	postmarkActivity := activities.NewPostmarkEmailActivity(
		...
		config.JWT.Secret,
		firstNonEmpty(config.Approvals.ApproveBaseURL, os.Getenv("GO_EMAIL_SERVER_BASE_URL"), "https://tenginex.zendwise.work"),
		log,
	)
...
func loadConfigFromEnv() *Config {
	return &Config{
		...
		Approvals: struct {
			...
			ApproveBaseURL: getEnvOrDefault("GO_EMAIL_SERVER_BASE_URL", "https://tenginex.zendwise.work"),
		},
		Logging: struct {
			Level  string `yaml:"level"`
			Format string `yaml:"format"`
